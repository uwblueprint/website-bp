...
interface SimpleMarqueeProps {
  ...
  /**
   * Scales the scroll-velocity contribution when `useScrollVelocity` is true
   * and `baseVelocity` is 0. Because the standard formula multiplies the base
   * movement by the scroll factor (and 0 × anything = 0), a non-zero scale is
   * required to make the marquee move at all when the base is zero.
   * Has no effect when `baseVelocity` is non-zero.
   * @default 20
   */
...
const SimpleMarquee = ({
  ...
  useAnimationFrame((t, delta) => {
    ...
    // When baseVelocity is 0 and scroll is enabled the standard multiplicative
    // formula (moveBy * velocityFactor) would always produce 0. Instead, add
    // the scroll contribution directly so scroll is the sole driver.
    if (useScrollVelocity && actualBaseVelocity === 0) {
      moveBy += velocityFactor.get() * (delta / 1000) * scrollVelocityScale
    } else {
